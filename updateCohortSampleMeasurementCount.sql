...
cohort_sample.condition_start_date, cohort_sample.condition_end_date, cohort_sample.comorbid_count, cohort_sample.drug_count, 
cohort_sample.procedure_count, cohort_sample.device_count, COUNT(measurement.measurement_id) 
FROM measurement
LEFT JOIN cohort_sample ON cohort_sample.person_id = measurement.person_id 
WHERE measurement.measurement_date BETWEEN substr(cohort_sample.condition_start_date, 7) || "-" || substr(cohort_sample.condition_start_date,4,2)  || "-" || substr(cohort_sample.condition_start_date, 1,2) AND substr(cohort_sample.condition_end_date, 7) || "-" || substr(cohort_sample.condition_end_date,4,2)  || "-" || substr(cohort_sample.condition_end_date, 1,2)
